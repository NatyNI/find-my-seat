...
const SearchInput = () => {
  ...
  return (
    <div className="flex flex-col items-center pt-24">
      {/* Top left corner */}
      <div className="absolute left-4 mt-[20px] h-6 w-6 border-l-4 border-t-4 border-green-500"></div>
      ...
      <input
        ...
        className="rounded-lg border border-green-500 px-4 py-2 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-500"
      />
      ...
      <div className="max-h-[288px] w-full max-w-[215px] overflow-y-auto pr-1">
        {sugestions.map((item, index) => (
          <div
            key={index}
            className="w-full rounded-md border-l-2 border-r-2 border-green-500/60 p-1.5 text-center font-bold hover:bg-green-500/40"
          >
            <ul>
              <li
                onClick={() =>
                  onClickReceiveNameAndTable(item.name, item.table, item.kids)
                }
              >
                {item.name}
              </li>
      ...
      <div className="fixed inset-0 -z-10 opacity-20">
        <img
          src="/images/bum.png"
          alt="A&M bumbac"
          className="h-full w-full translate-y-10 object-cover"
        />
      </div>

      {/* Bottom left corner*/}
      <div className="absolute bottom-[90px] left-4 h-6 w-6 border-b-4 border-l-4 border-green-500"></div>

      {/* Bottom right corner*/}
      <div className="absolute bottom-[90px] right-4 h-6 w-6 border-b-4 border-r-4 border-green-500"></div>
    </div>
  );
};
